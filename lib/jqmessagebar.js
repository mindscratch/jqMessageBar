(function($) {
	...
	var api = {
		...
		showNext: function() {
			...
			return this.each(function() {
				...
				if (index < (msgCount - 1)) {
					// show up arrow
					toggleArrow(upArrow, true, false);
		...
		showPrevious: function() {
			...
			return this.each(function() {
				...
				if (index > 0) {
					// show down arrow
					toggleArrow(downArrow, false, false);
	...
	*/	
	...
	var displayMessage = function(message, settings, jqPluginEl, jqBarEl, jqContentEl) {
		...
		jqContentEl.addClass(contentClass).text(message.content());
	};
	
	var toggleArrow = function(arrow, up, hide) {
		// summary:
